...
TINY_EMBREE_API void SetKnnPoints(void* accelerator, Point* data,
...
TINY_EMBREE_API Neighbor* KnnQuery(void* accelerator, void* cache,
                                   const Point* pos, float radius,
                                   unsigned int k, unsigned int* numFound) {
    ((tinyembree::KNNResult*)cache)->k = k;
    ((tinyembree::KNNResult*)cache)->knn.clear();
    ((tinyembree::PointQuery*)accelerator)
        ->KnnQuery(pos, radius, ((tinyembree::KNNResult*)cache));
    *numFound = ((tinyembree::KNNResult*)cache)->knn.size();
    return ((tinyembree::KNNResult*)cache)->knn.data();
}
